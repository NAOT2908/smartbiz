...
export class DialogModal extends Component {
  ...
  getInputValue(field){
    const v=this.state.form[field.name];
    if(field.type==="datetime-local"&&v) return utcToLocalInput(v,this.tz);
    if(field.type==="select") return (v===null||v===undefined)?"":String(v);
    return v ?? "";
  }
  _onInput(name,ev){
    let val=ev.target.value;
    if(ev.target.type==="datetime-local"&&val){
      val=localInputToUtc(val,this.tz);
