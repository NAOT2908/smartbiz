...
export class smartbizBarcodeKanbanController extends KanbanController {
    setup() {
    ...
    openRecord(record) {
        ...
        {
            this.actionService.doAction('smartbiz_barcode_stock.stock_picking_client_action', {
        ...
        {
            this.actionService.doAction('smartbiz_barcode_stock.stock_picking_batch_client_action', {
                additionalContext: { active_id: record.resId,resModel: record.resModel
                 },
            });
        }
    ...
    async createRecord() {
        if (this.props.resModel === 'stock.picking')
        {
            ...
            if (id) {
                return this.actionService.doAction('smartbiz_barcode_stock.stock_picking_client_action', {
        ...
        if (this.props.resModel === 'stock.picking.batch')
        {
            ...
            if (id) {
                return this.actionService.doAction('smartbiz_barcode_stock.stock_picking_batch_client_action', {
                    additionalContext: { active_id: id,resModel: this.props.resModel
                     },
                });
            }
    ...
    async _onBarcodeScannedHandler(barcode) {
        ...
        if (res.action) {
            ...
            return this.actionService.doAction(res.tag, {
                additionalContext: res.params
            });
        }
        else if (res.warning) {
