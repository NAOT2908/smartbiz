...
mapOvertimeStatus(state) {
  ...
  isAllSelected() {
    return this.state.selectedIds.length === this.state.attendance_data.length && this.state.attendance_data.length > 0;
  }
    // =====================
...
showModal(title, action = '', defaultValues = null) {
    const formMap = {
        ...
        overtime_data: [
            {name    : "employee_ids", type    : "multiselect", label   : "Nhân viên", required: true,
            columns : [
                { key: "name",  label:"Nhân viên" },
                { key: "barcode", label:"Mã" },
                { key: "department_id", label:"Phòng ban" },
  ...
  async getData(id) {
    ...
    this.employees = await this.orm.searchRead(
          'hr.employee', [['active', '=', true]], ['id', 'name', 'barcode', 'work_email', 'pin', 'department_id']
      );
