...
export class MainMenu extends Component {
    ...
    async openPickingType() {
        ...
        const res = await this.rpc('/smartbiz_barcode/open_view', { name:"Kiểu điều chuyển",res_model:'stock.picking.type',view_id:'smartbiz_barcode.stock_picking_type_kanban',view_mode:'kanban',context:context });
        if (res) {
            return this.actionService.doAction(res.action);
        }
    }
    async openPickingBatch() {
        ...
        const res = await this.rpc('/smartbiz_barcode/open_view', { name:"Điều chuyển loạt",res_model:'stock.picking.batch',view_id:'smartbiz_barcode.stock_picking_batch_kanban',view_mode:'kanban',context:context });
        if (res) {
            return this.actionService.doAction(res.action);
        }
    }
    ...
    async openPop() {
        ...
        const res = await this.rpc('/smartbiz_barcode/open_view', { name:"Lệnh sản xuất",res_model:'mrp.production',view_id:'smartbiz_barcode.mrp_production_kanban',view_mode:'kanban',context:context });
        if (res) {
            return this.actionService.doAction(res.action);
        }
    }
    async openWorkorder() {
        await this.actionService.doAction("smartbiz_barcode.mrp_workorder_action");
    }
